...
## band combination landsat 
## ANYTHING LIKE c("red","green","blue")  c("swir1","nir","red") c("nir","red","green")
lsat_band_combination <- c("red","green","blue") 
  

## band combination sentinel 
stnl_band_combination <- c("nir","red","green")
...
for(the_id in listodo[1:to_go]){
  ...
  for(year in c(yr_str_lsat:yr_end_lsat)){
    ...
    tryCatch({
      ...
      swir <- raster(lsat_clip,which(names(lsat)== "swir2"))
      nir  <- raster(lsat_clip,which(names(lsat)== "nir"))
      red  <- raster(lsat_clip,which(names(lsat)== "red"))
      
      ndvi <- (nir-red)/(nir+red)
      ndwi <- (nir-swir)/(nir+swir)
      
      lsat_R <- raster(lsat_clip,which(names(lsat)== lsat_band_combination[1]))
      lsat_G <- raster(lsat_clip,which(names(lsat)== lsat_band_combination[2]))
      lsat_B <- raster(lsat_clip,which(names(lsat)== lsat_band_combination[3]))
      ...
      stackLSAT <- stack(lsat_R,lsat_G,lsat_B)
      
      plotRGB(stackLSAT,stretch="hist",add=T)
      #plot(ndvi,add=T)
    ...
    rect(
      ...
      xright =  margins@xmin + outside_box_size/3/111320, 
      ytop =    margins@ymax, 
      col = "white", 
      border = NA)
  ...
  for(year in c(yr_str_stnl:yr_end_stnl)){
    ...
    tryCatch({
      ...
      swir <- raster(stnl_clip,which(names(stnl)== "swir2"))
      nir  <- raster(stnl_clip,which(names(stnl)== "nir"))
      red  <- raster(stnl_clip,which(names(stnl)== "red"))
      
      ndvi <- (nir-red)/(nir+red)
      ndwi <- (nir-swir)/(nir+swir)
      
      stnl_R <- raster(stnl_clip,which(names(stnl)== stnl_band_combination[1]))
      stnl_G <- raster(stnl_clip,which(names(stnl)== stnl_band_combination[2]))
      stnl_B <- raster(stnl_clip,which(names(stnl)== stnl_band_combination[3]))
      ...
      stackSTNL <- stack(stnl_R,stnl_G,stnl_B)
      
      plotRGB(stackSTNL,stretch="hist",add=T)
